...
public abstract class FileDataExtractorExcel extends FileDataExtractor {
	...
	public Map<Integer, List<String>> getExcelFile(DataExtractContext context, String key, String filePath [], String title,
		...
		try {
			File file = download(context, title, filePath);
			if (file == null) {
				throw new RuntimeException(MessageFormat.format("\"{0}\" 엑셀파일을 찾지 못하였습니다.!", "상품코드 LIST"));
			}

			excelData = extractDataFromExcel(context, file, key, sheetName, startCol, startRow);
	...
	@SuppressWarnings("unchecked")
	public Map<Integer, List<String>> getExcelFileEx(DataExtractContext context, String key, String filePath [], String title,
			String sheetName, int[] startCol, int startRow) {
		...
		try {
			...
			excelData = extractDataFromExcelEx(context, file, key, sheetName, startCol, startRow);
		} catch (RuntimeException e) {
			throw e;
		} catch (Exception e) {
			throw new RuntimeException(MessageFormat.format("\"{0}\" 엑셀파일 파싱 실패!", "상품코드 LIST"), e);
	...
	public Map<Integer, List<String>> extractDataFromExcelEx(DataExtractContext context,
	...
	public Map<Integer, List<String>> extractDataFromExcel(DataExtractContext context,
			File file, String key, String sheetName, int startCol, int startRow) {
		// Row 단위로 데이터를 추출하여 context에 저장
