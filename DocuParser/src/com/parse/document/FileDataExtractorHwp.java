...
public abstract class FileDataExtractorHwp extends FileDataExtractor {
	...
	public HWPFile[] getHWPFile(DataExtractContext context, String title) {
		...
		try {
			File[] files = downloads(context, title, HWP);
			if (files != null) {
				for (File file : files) {
	...
	public void processDocument(DataExtractContext context, String title,
		...
		HWPFile[] hwpFiles = getHWPFile(context, title); // HWP 파일 배열을 가져옴
	...
	public void processDocumentEx(DataExtractContext context, String title,
		...
		HWPFile[] hwpFiles = getHWPFile(context, title);
